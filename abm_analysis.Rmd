...
for (runname in simruns) {
  ...
  current_agent_log %>% 
    filter(TickNr == 1000) %>% 
    .$Opinion -> agent_opinions
  
  V(current_graph)$opinion <- agent_opinions
  write.graph(current_graph, paste0(runname,".gml"), format="gml")
}

BAv2_run11_agent_log %>% 
  filter(AgentID == 369)

get(paste0(simruns[1], "_agent_log")) %>% 
    filter(TickNr == 1000) %>% 
    .$Opinion -> agent_opinions
  
  V(get(paste0(simruns[1], "_graph_final")))$opinion <- agent_opinions

BAv2_run15_agent_log %>% 
  filter(TickNr == 1000) %>% 
  .$Opinion -> agent_opinions

V(BAv2_run15_graph_final)$opinion <- agent_opinions
write.graph(BAv2_run15_graph_final, "graph_test2.gml", format="gml")
  
test <- E(BAv2_run15_graph_final)$arrow.size
E(BAv2_run15_graph_final)$arrow.size <- 0.1
E(BAv2_run15_graph_final)$weight <- 1.0
...
g <- make_ring(10) %>%
  set_edge_attr("width", value = 1:10) %>%
