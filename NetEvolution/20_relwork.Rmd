...
This section supplies the prerequisites for creating such a simulation model by looking on the dynamics that occur, indicating how they can be modelled and investigating how the surrounding network can be replicated appropriately.
...
Nowadays, those services provide far more features as news media entered this environments and people started to not only share their everyday activities and cat photos, but also are voicing now their opinions and perceiving the reactions of others.
...
While this increased exposure could be suspected as positive influence on the opinion formation, this type of media also shows more vulnerability for misleading the public discourse on certain topics like it got evident for events as the Brexit, .., or most recently the spread of wrong information about the corona virus [@McCloskey2020].

Further on it is crucial to consider the imbalance of activity of social media users. 
As Romero et al. found, the majority of users passively consume content on social media platforms and rarely take part in interactions, while only a little part of the users utilize those platforms to actively contribute new content and spread their opinions which transforms them into a kind of secondary gate keepers of information spread. 
Those individuals are also referred to as opinion leaders. 
...
This serves as motivation to spend a closer look on the underlying network structure of social media platforms as the structure impacts both active and passive users equally. 
...
With higher clustering for example the effect of polarization increases, while it has no effect on the spread of misinformation. 
Regarding centrality it is the other way round: if potential spreaders of misinformation occupy positions in the network with high followercount, the dissemination of misinformation is facilitated whereas the effect on polarization shows no significance [@NBERw24462].
...
It is necessary to transform the opinion of a social network member into a value that allows for transforming and comparing to others based on mathematical operations. 
Only this way we can implement mechanisms for the interaction and mutual influence of opinions inside the simulated network community.
...
@Degroot1974 et al. therefore replicated the beliefs into subjective probability distributions which allowed to perform the required calculations that are modelling the opinion formation.
...
In comparison with a model that uses complete mixing of agents, the network version shows especially for low opinion thresholds that consensus is no longer found for a major group of agents but rather depends on the connectedness of agent clusters [@Deffuant2000].
...
It could be shown, that such supernodes were most influential compared to all other nodes and processed themselves also a significant opinion change during the clustering process. With decreasing density, differences to the standard mixed variant get more visible.
...
Studies of Fortunato et al. show equal indications that for epsilon < 0.5 an opinion dynamics model based on the approach of Deffuant et al. always leads to the formation of a consensus independent from the underlying network structure [@Weisbuch2004].
...
To reproduce an online social network in a simulation we must rely on a network generator that is comparable to a real network. 
As social media platforms such as Facebook and Twitter are close to a scale-free network in their overall network structure and follow a powerlaw distribution, the Barabasi-Albert network generator is most suitable for generating a realistic network topology. 
Within a Barabasi-Albert network there exists a little amount of very well-connected hubs while most nodes have only few connections to others. 
The Barabasi-Albert generator provides several parameters like the initial network size, the number of new nodes that are added to an initial network and the number of edges which are created by the joining nodes to existing nodes using preferential attachment. 
This allows us to get the simulation close to a real social network structure.