...
# Results
Our simulation study comprised in total 27 simulation runs that show the impact and relationships of the varied factors on the network evolution and opinion dynamics.
...
Looking on the amount of edges relative to the complete network edge count for the certain network size, the coverage of edges was highest in the simulations with lowest agent count (n=100) with a mean coverage of 11.09%, not regarding the other factors. The other two settings showed mean coverage of 9.04% for 500 agents and and 8.83% for 1000 agents. The network shows also a significant correlation with the standard deviation of the outdegree when looking on the mean values for the 9 distinct simulations of the particular network size. The bigger the network, the higher the deviations in the distribution of outdegree from the mean outdegree in a network. With bigger network size also the mean centrality of a node increases.

## Impact of the Unfriend Threshold
...
Regarding the approach how agents create new edges the most prominent impact on the network evolution showed in 
Partial consensus calculation? Neighbors of neighbors seems to have higher partial agreement in opinion than random and hybrid approach
