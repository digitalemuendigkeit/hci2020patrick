...
```{r Combined Evaluation}
# Density

results %>% 
  select(BatchName,ConfigAgentCount, ConfigUnfriendThresh, ConfigAddfriendMethod, 
         Densities, OutdegreeIndegreeRatioMean, ClosenessCentralityMean, BetweennessCentralityMean, OpinionSD) %>% 
  mutate(RunName = case_when(BatchName == "Addfriends" ~ case_when(ConfigAddfriendMethod == "neighborsofneighbors" ~ "Neighbors",
                                                                   TRUE ~ as.character(ConfigAddfriendMethod)),
                             BatchName == "Netsize" ~ paste0(ConfigAgentCount),
                             BatchName == "Unfriend" ~ paste0(ConfigUnfriendThresh)),
         RunNr = case_when(BatchName == "Addfriends" ~ as.numeric(ConfigAddfriendMethod),
                           BatchName == "Netsize" ~ as.numeric(ConfigAgentCount),
                           BatchName == "Unfriend" ~ as.numeric(ConfigUnfriendThresh))) %>% 
  group_by(BatchName, RunNr, RunName) -> eval

# Density

eval %>% 
  summarise(Mean_Density = mean(Densities),
            SD_Density = sd(Densities)) %>% 
  ggplot(aes(x = reorder(RunName, RunNr), y = Mean_Density, ymin = Mean_Density - SD_Density, ymax = Mean_Density + SD_Density)) +
  geom_bar(stat = "identity", aes(fill = RunNr)) +
  geom_errorbar(position=position_dodge(0.9),width = 0.4) +
  facet_grid(~BatchName, scales = "free") +
  scale_fill_viridis_c(begin = 0.2) +
  labs(x="Name of the Configuration", y="Mean Density of a Run in percent", 
       title="Network densities ") +
  guides(fill = FALSE) +
  NULL
...
eval %>% 
  summarise(Mean = mean(ClosenessCentralityMean),
            SD = sd(ClosenessCentralityMean)) %>% 
  ggplot(aes(x = reorder(RunName, RunNr), y = Mean, ymin = Mean - SD, ymax = Mean + SD)) +
  geom_bar(stat = "identity", aes(fill = RunNr)) +
  geom_errorbar(position=position_dodge(0.9),width = 0.4) +
  facet_grid(~BatchName, scales = "free") +
  scale_fill_viridis_c(begin = 0.2) +
  labs(x="Name of the Configuration", y="Mean Centrality of a Run", 
...
eval %>% 
  summarise(Mean = mean(BetweennessCentralityMean),
            SD = sd(BetweennessCentralityMean)) %>% 
  ggplot(aes(x = reorder(RunName, RunNr), y = Mean, ymin = Mean - SD, ymax = Mean + SD)) +
  geom_bar(stat = "identity", aes(fill = RunNr)) +
  geom_errorbar(position=position_dodge(0.9),width = 0.4) +
  facet_grid(~BatchName, scales = "free") +
...
results %>% 
  select(BatchName,ConfigAgentCount, ConfigUnfriendThresh, ConfigAddfriendMethod, 
         ClosenessCentralityMean, BetweennessCentralityMean, EigenCentralityMean) %>% 
  mutate(RunName = case_when(BatchName == "Addfriends" ~ case_when(ConfigAddfriendMethod == "neighborsofneighbors" ~ "Neighbors",
                                                                   TRUE ~ as.character(ConfigAddfriendMethod)),
                             BatchName == "Netsize" ~ paste0(ConfigAgentCount),
                             BatchName == "Unfriend" ~ paste0(ConfigUnfriendThresh)),
...
eval %>% 
  summarise(mean = mean(ClosenessCentralityMean),
            sd = sd(ClosenessCentralityMean)) %>% 
  mutate(CurrentVar = "ClosenessCentralityMean") -> eval_closeness

eval %>% 
  summarise(mean = mean(BetweennessCentralityMean),
            sd = sd(BetweennessCentralityMean)) %>% 
  mutate(CurrentVar = "BetweennessCentralityMean") -> eval_betweenness

eval %>% 
  summarise(mean = mean(EigenCentralityMean),
            sd = sd(EigenCentralityMean)) %>% 
  mutate(CurrentVar = "EigenVectorCentrality") -> eval_eigencentrality


bind_rows(eval_closeness, eval_betweenness, eval_eigencentrality) %>% 
  ggplot(aes(x = reorder(RunName, -RunNr), y = mean, ymin = mean - sd, ymax = mean + sd)) +
  coord_flip() +
  facet_grid(BatchName~CurrentVar, scales = "free", switch = "y", labeller = label_wrap_gen(width=10)) +
  geom_bar(stat = "identity", aes(fill = RunNr)) +
  geom_errorbar(position=position_dodge(0.9),width = 0.4) +
  ...
  labs(x="Name of the Configuration", y="", 
       title="Centrality") +
  guides(fill = FALSE) +
  scale_x_discrete(position="top", labels = function(x) lapply(strwrap(x, width = 14, simplify = FALSE), paste, collapse="\n")) +
...
results %>% 
  ...
  labs(x="Name of the Configuration", title="Densities") +
  scale_fill_viridis_c()
...
results %>% 
  ...
  geom_boxplot(alpha = 0.5, aes(fill = Xlevel)) +
  theme(legend.position = "none") +
  labs(x="Name of the Configuration", title="Outdegree SD") +
  scale_fill_viridis_c()
...
#   geom_boxplot(alpha = 0.5, aes(fill = Xlevel)) +
#   theme(legend.position = "none") +
#   labs(x="Name of the Configuration", title="Outdegree Mean relative to Network Size") +
#   scale_fill_viridis_c()
...
results %>% 
  ...
  geom_boxplot(alpha = 0.5, aes(fill = Xlevel)) +
  theme(legend.position = "none") +
  labs(x="Name of the Configuration", title="Indegree SD") +
  scale_fill_viridis_c()
...
results %>% 
  ...
  geom_boxplot(alpha = 0.5, aes(fill = Xlevel)) +
  theme(legend.position = "none") +
  labs(x="Name of the Configuration", title="Outdegree Indegree Ratio") +
  scale_fill_viridis_c()
...
results %>%
  ...
  geom_boxplot(alpha = 0.5, aes(fill = Xlevel)) +
  theme(legend.position = "none") +
  labs(x="Name of the Configuration", title="Closeness Centrality") +
  scale_fill_viridis_c()
...
results %>%
  ...
  geom_boxplot(alpha = 0.5, aes(fill = Xlevel)) +
  theme(legend.position = "none") +
  labs(x="Name of the Configuration", title="Betweenness Centrality") +
  scale_fill_viridis_c()
...
results %>%
  ...
  geom_boxplot(alpha = 0.5, aes(fill = Xlevel)) +
  theme(legend.position = "none") +
  labs(x="Name of the Configuration", title="Eigenvector Centrality") +
  scale_fill_viridis_c()
...
results %>%
  ...
  geom_boxplot(alpha = 0.5, aes(fill = Xlevel)) +
  theme(legend.position = "none") +
  labs(x="Name of the Configuration", title="Cluster Coefficient") +
  scale_fill_viridis_c()

results %>%
  ggplot(aes(x = reorder(Xlabel, Xlevel), y = CommunityCount)) +
  facet_wrap(~BatchName, scales = "free_x") +
  
  geom_violin(alpha = 0.1, fill = "black") +
  geom_boxplot(alpha = 0.5, aes(fill = Xlevel)) +
  theme(legend.position = "none") +
  labs(x="Name of the Configuration", title="Community Count") +
  scale_fill_viridis_c()
...
results %>%
  ...
  geom_boxplot(alpha = 0.5, aes(fill = Xlevel)) +
  theme(legend.position = "none") +
  labs(x="Name of the Configuration", title="Connected Components") +
  scale_fill_viridis_c()
...
results %>%
  ...
  geom_boxplot(alpha = 0.5, aes(fill = Xlevel)) +
  theme(legend.position = "none") +
  labs(x="Name of the Configuration", title="Opinion SD") +
  scale_fill_viridis_c()
...
results %>%
  ...
  geom_boxplot(alpha = 0.5, aes(fill = Xlevel)) +
  theme(legend.position = "none") +
  labs(x="Name of the Configuration", title="Opinion Change Delta") +
  scale_fill_viridis_c()
...
results %>%
  ...
  geom_boxplot(alpha = 0.5, aes(fill = Xlevel)) +
  theme(legend.position = "none") +
  labs(x="Name of the Configuration", title="Difference between perceived public and own opinion") +
  scale_fill_viridis_c()
...
results %>%
  ...
  geom_boxplot(alpha = 0.5, aes(fill = Xlevel)) +
  theme(legend.position = "none") +
  labs(x="Name of the Configuration", title="Community Opinion Differences") +
  scale_fill_viridis_c()
...
results %>%
  ...
  geom_boxplot(alpha = 0.5, aes(fill = Xlevel)) +
  theme(legend.position = "none") +
  labs(x="Name of the Configuration", title="Supernode Outdegree relative to Net size") +
  scale_fill_viridis_c()
...
results %>%
  ...
  geom_boxplot(alpha = 0.5, aes(fill = Xlevel)) +
  theme(legend.position = "none") +
  labs(x="Name of the Configuration", title="Supernode Closeness") +
  scale_fill_viridis_c()
...
results %>%
  ...
  geom_boxplot(alpha = 0.5, aes(fill = Xlevel)) +
  theme(legend.position = "none") +
  labs(x="Name of the Configuration", title="Supernode Betweenness") +
  scale_fill_viridis_c()
...
results %>%
  ...
  geom_boxplot(alpha = 0.5, aes(fill = Xlevel)) +
  theme(legend.position = "none") +
  labs(x="Name of the Configuration", title="Supernode Eigenvector") +
  scale_fill_viridis_c()
...
results %>%
  ...
  geom_boxplot(alpha = 0.5, aes(fill = Xlevel)) +
  theme(legend.position = "none") +
  labs(x="Name of the Configuration", title="Supernode Opinion") +
  scale_fill_viridis_c()
